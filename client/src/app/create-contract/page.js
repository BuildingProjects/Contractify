...
export default function CreateContractPage() {
  ...
  const validateForm = () => {
    ...
    // Email validation
    if (formData.contractorEmail && !/\S+@\S+\.\S+/.test(formData.contractorEmail)) {
      newErrors.contractorEmail = 'Invalid contractor email format';
    }
    if (formData.contracteeEmail && !/\S+@\S+\.\S+/.test(formData.contracteeEmail)) {
      newErrors.contracteeEmail = 'Invalid contractee email format';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(responseData.error || responseData.details || "Failed to create contract. Please try again.");
      }
    } catch (error) {
    ...
    <div className='min-h-screen bg-gray-50'>
      ...
      <main className='pt-16 sm:pt-20 px-3 sm:px-6 lg:px-8 max-w-4xl mx-auto pb-12'>
        ...
        >
          ...
          {/* Download Button - Only show after successful creation */}
          {createdContractId && (
            <div className='mt-4'>
              <button
                type='button'
                ...
                className='w-full bg-green-600 text-white py-2.5 px-4 rounded-lg hover:bg-green-700 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 shadow-md flex items-center justify-center gap-2'
              >
